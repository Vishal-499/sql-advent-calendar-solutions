...
WITH top_rank_table as (SELECT *, Rank() OVER(PARTITION BY target_name ORDER BY evilness_score desc ,created_at desc ) as x
  FROM grinch_prank_ideas ORDER BY evilness_score desc)
  SELECT prank_id,target_name,prank_description, evilness_score, created_at
  FROM top_rank_table WHERE x = 1;
